...
GUIb GUI::update(const GNCb& gnc) {
    ...
    if (gnc.vsmb.att_mode == ATT_MODE::RATE) {
        guib.omega_body_radps.x() = roll_expo * cfg_data.guic.max_rate_radps;
        guib.omega_body_radps.y() = pitch_expo * cfg_data.guic.max_rate_radps;
        guib.omega_body_radps.z() = yaw_expo * cfg_data.guic.max_rate_radps;
        guib.euler_bodyz2up_rad = Eigen::Vector2f::Zero();
    } else if (gnc.vsmb.att_mode == ATT_MODE::ANGLE) {
        guib.euler_bodyz2up_rad.x() = roll_expo * cfg_data.guic.max_angle_rad;
        guib.euler_bodyz2up_rad.y() = pitch_expo * cfg_data.guic.max_angle_rad;
        guib.omega_body_radps.z() = yaw_expo * cfg_data.guic.max_rate_radps;
